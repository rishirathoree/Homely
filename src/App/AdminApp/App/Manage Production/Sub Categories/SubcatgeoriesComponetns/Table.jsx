...
import { DltSubCategory, UpdateCatgeoriesStatus } from '../../../../../../Store/Slices/Subcategories'
...
const Table = ({subCategory}) => {
  ...
  const {pending,data,error} = subCategory

  const [AllSubCategory,setAllSubCategory] = useState([])

  useEffect(()=>{setAllSubCategory(data)},[data])
  
  const toggleActivation = (idx,itemId) => {
    setAllSubCategory((prevProducts) => {
  ...
  const handleDeleteCategory = (CtgId) => {
    ...
    const findCategory = AllSubCategory.findIndex((item) => item.id === CtgId);

    const newUpdatedArray = [...AllSubCategory];

    newUpdatedArray.splice(findCategory, 1);

    setAllSubCategory(newUpdatedArray);

    dispatch(DltSubCategory(CtgId));
          ...
          <table className='w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400'>
            <thead className='text-xs text-gray-700 uppercase dark:text-gray-400'>
              <tr className='border-b-[1px]'>
                ...
                <th className='px-6 w-40 py-3 text-[10px]'>Name</th>
            ...
            <tbody>
              {AllSubCategory?.map((item, idx) => (
                ...
                >
                  ...
                  <td className='px-6 py-4'>{item.description.slice(0,150)}...</td>
                  ...
                  <td className='px-6 mt-1 py-4 gap-2'>
                    <div className='flex'>
